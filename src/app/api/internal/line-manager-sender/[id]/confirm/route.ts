...
import { confirmLineManagerSenderWork, LineManagerSendOutboxError, LineManagerSenderInputError, parseLineManagerSenderId, parseLineManagerSenderJson } from "@/lib/line-manager-sender-internal";
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (error instanceof LineManagerSendOutboxError) return NextResponse.json({ ok: false, error: error.message }, { status: 409 });
    throw error;
  }
}
