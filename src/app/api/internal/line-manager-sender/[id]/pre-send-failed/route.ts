...
import { acknowledgeLineManagerPreSendFailure, LineManagerSenderInputError, parseLineManagerSenderId, parseLineManagerSenderJson } from "@/lib/line-manager-sender-internal";
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const ok = await acknowledgeLineManagerPreSendFailure(prisma, parseLineManagerSenderId(params.id), await parseLineManagerSenderJson(request));
