    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {isLoading ? (
          Array(6)
            .fill(0)
            .map((_, i) => <div key={i} className="h-40 bg-zinc-100 animate-pulse rounded-lg" />)
        ) : (
          customers.map((c) => (
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="font-bold text-zinc-900 flex items-center gap-2">
                      {c.name}
                      {c.isPartner && <Badge className="bg-blue-100 text-blue-700 hover:bg-blue-100 border-0 text-[10px] h-4">パートナー</Badge>}
                    </div>
                    <div className="text-[10px] text-zinc-400 uppercase tracking-tighter">
                      ID: {c.type === "business" && c.prefix ? `${c.prefix}-` : ""}
                      {c.id}
                    </div>
                  </div>
                </div>
