...
export default function CustomerEditPage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        getCustomerById(id).then(res => {
            if (res) {
                ...
                toast({ title: "Error", description: "顧客が見つかりません", variant: "destructive" });
    ...
    if (isLoading) return <div className="p-8 text-center text-zinc-400">Loading...</div>;
    if (!data) return null;
    ...
    return (
        <div className="p-4 md:p-8 max-w-3xl mx-auto space-y-6">
            ...
            <form onSubmit={handleSave} className="space-y-6">
                <Card className="p-6 space-y-4 border-zinc-200 shadow-sm">
                    <div className="flex bg-zinc-100 p-1 rounded-md mb-6 max-w-[240px]">
                        <button
                            ...
                            className={cn("flex-1 text-xs font-bold py-1.5 rounded-sm transition-all flex items-center justify-center gap-1", data.type === "business" ? "bg-white shadow-sm text-blue-600" : "text-zinc-400")}
                        >
                        ...
                        <button
                            ...
                            className={cn("flex-1 text-xs font-bold py-1.5 rounded-sm transition-all flex items-center justify-center gap-1", data.type === "individual" ? "bg-white shadow-sm text-green-600" : "text-zinc-400")}
                        >
                            <User className="w-3 h-3" /> 一般 (B2C)
                        </button>
                    </div>
                ...
                {data.type === "business" && (
                    <Card className="p-6 space-y-4 border-l-4 border-l-blue-500 shadow-sm border-zinc-200">
                        <h3 className="text-sm font-bold flex items-center gap-2 text-zinc-700">取引先（パートナー）設定</h3>
                        <div className="flex items-center gap-4">
                            <div className="flex items-center gap-2">
                                <input
                                    ...
                                    className="w-4 h-4"
                                />
                                <Label htmlFor="isPartner" className="text-sm cursor-pointer font-bold">公式パートナーとして扱う</Label>
                            </div>
                            {data.isPartner && (
                                <div className="flex items-center gap-2">
                                    <Label className="text-xs font-bold text-zinc-500">管理番号プレフィックス:</Label>
                                    <Input
                                        value={data.prefix || ""}
                        ...
                        <p className="text-[10px] text-zinc-400 italic">※パートナー設定を有効にすると、修理受付時に独自の管理番号（例: {data.prefix || "T"}-001）が自動採番されます。</p>
                    </Card>
                )}

                <div className="flex justify-end gap-3 pt-4">
                    <Button type="button" variant="ghost" onClick={() => router.push("/customers")} className="text-zinc-500">キャンセル</Button>
            ...
            <div className="pt-12 border-t">
                <h3 className="text-sm font-bold text-zinc-400 uppercase tracking-widest mb-4">最近の修理履歴</h3>
                <div className="space-y-2">
                    ...
                    {(!data.repairs || data.repairs.length === 0) && <p className="text-zinc-400 italic text-xs">履歴はありません</p>}
                </div>
            </div>
        </div>
    );
