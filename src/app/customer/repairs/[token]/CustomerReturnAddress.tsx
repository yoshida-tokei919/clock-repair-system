...
function normalizePostalCode(value: string) {
  ...
  if (!/^\d{7}$/.test(normalized) && !/^\d{3}[-‐‑‒–—―−ー－]\d{4}$/.test(normalized)) return null;
...
export function CustomerReturnAddress({ token, address: initialAddress, disabled, onApprovalStateChange }: Props) {
  ...
  return <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h2 className="text-lg font-bold">返送先</h2><p className="mt-1 text-sm text-slate-500">承認前に返送先をご確認ください。</p></div>{!disabled && !editing && <button type="button" onClick={() => setEditing(true)} className="rounded-md border border-blue-200 px-3 py-2 text-sm font-bold text-blue-700">返送先を変更する</button>}</div>{editing ? <><AddressFields address={address} setAddress={setAddress} onPostalBlur={() => void lookupPostalCode()} /><div className="mt-4 flex gap-2"><button type="button" onClick={() => void save()} disabled={saving} className="rounded-md bg-blue-600 px-4 py-2 text-sm font-bold text-white disabled:opacity-50">{saving ? "保存中…" : "変更を保存する"}</button><button type="button" onClick={() => { setAddress(savedAddress); setEditing(false); setMessage(""); }} disabled={saving} className="rounded-md border border-slate-300 px-4 py-2 text-sm font-bold">キャンセル</button></div></> : <div className="mt-4 rounded-lg bg-slate-50 p-3 text-sm leading-6 text-slate-800"><div className="font-bold">{address.recipientName || "未入力"}</div><div>〒{address.postalCode || "未入力"}</div><div>{[address.prefecture, address.city, address.street, address.building].filter(Boolean).join("") || "未入力"}</div><div>TEL {address.phone || "未入力"}</div></div>}{message && <p className="mt-3 text-sm text-slate-600">{message}</p>}</section>;
}
