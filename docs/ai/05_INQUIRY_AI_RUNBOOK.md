...
## LINE Manager sender local worker (no real sending in this task)
...
- The orchestration/reconciliation algorithm is implemented and unit-tested with typed/fake `HistoryMessage` data. The real lineoa raw-history parser is deliberately not enabled: lineoa `7.7.18` returns raw JSON, but no authenticated raw history shape, stable schema, or fixture has been captured and verified. Do not guess raw fields and do not claim live OUTBOUND or sendId availability. Once a verified parser exists, typed history with any `sendId` values must require frozen chat ID + exact sendId + exact text + actual message ID; otherwise use only a timezone-aware window strictly after `max(pre_send_watermark, captured_at)` and ending at server `postAttemptedAt` plus the bounded allowance, failing closed on ambiguity.
- This task has a local LINELib sender but real sending must not be enabled yet: no real LINE send, production E2E/deploy, reply UI, or DC/R2 removal is authorized. The next approved step is authenticated history-schema capture/verification (prefer a non-sending history read if feasible), then parser implementation from the observed verified shape, then controlled production E2E under explicit approval. Production remains pending for `a8628d0`.
