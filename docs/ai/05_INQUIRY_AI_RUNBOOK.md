...
## Production rollout state — 2026-09-24

- GitHub `main` and Railway production are on `38d38c09109a880767ba4e4c6802e890cfdee7bf`.
- Railway deployment `9869644f-e192-4f62-a244-ddd9b707b25e` completed with `SUCCESS`; the instance is `RUNNING`.
- Production Prisma migrations are fully current at 27 applied / 0 unfinished, including the Task 2/3/4 migrations below.
- A production backup was taken before migration at `C:\Users\yoshi\clock-repair-backups\production-20260923T233823Z`.
- Non-sending smoke succeeded: `/` 200, `/login` 200, unauthenticated `/api/repairs/1/line` 401.
- No real LINE send was performed during rollout smoke.
- Yoshida's authenticated production screen verification of Inquiry LINE and Repair LINE is still pending.
- Create the production tag only after that manual screen verification is complete.
...
- Task 2, Task 3, and Task 4 migrations were applied to production on 2026-09-24. Task 4 only alters the existing server-internal outbox table, so it adds no Data API grant.
...
- The table is server-internal and has no browser Data API grants. The Task 3 migration explicitly revokes `PUBLIC`, `anon`, and `authenticated` privileges. Task 2 and Task 3 migrations were applied to production on 2026-09-24.
