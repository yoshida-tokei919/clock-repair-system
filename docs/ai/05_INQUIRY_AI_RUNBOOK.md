...
- This task has a local LINELib sender but real sending is still not enabled or approved: no real LINE send, production E2E/deploy, reply UI, or DC/R2 removal is authorized. Production remains pending; the sender internal API, local sender, and history parser are not deployed, while only the `e13b068` foundation is production-complete. Authenticated read-only live history was verified, and the implemented parser was independently run successfully against the captured live response. A non-sending blocker remains: there is no persistent lineoa cookie storage file (for example, `lineoa-storage.json`) under the POC root. Authentication for read-only inspection used a dedicated browser profile, whereas the local sender factory expects an existing lineoa storage path. Next: (1) establish or verify persistent authenticated lineoa storage/session without sending; then (2) conduct controlled E2E send only with explicit user approval. Do not implement the auth-storage step in this task.
