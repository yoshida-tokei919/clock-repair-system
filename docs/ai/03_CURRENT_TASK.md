...
## Task: persistent lineoa auth storage bootstrap and verification

Foundation at `e13b068254e9596be5ee8ef60f20509d8d14ec84` is production complete. This Task is based on `27b8f4c278ae9e20146de0b5de8ea651d96c87a4` and adds only a reusable, non-sending persistent lineoa auth-storage bootstrap/verification utility.

Production: pending. The sender internal API, local sender, history parser, and this auth-storage utility have not been deployed; only the earlier `e13b068` foundation is production complete. This Task performs no real LINE send, deploy, push, schema change, environment change, or secret change.
...
Real sending is still not enabled or approved, and production remains pending. On 2026-09-23, independent non-sending live verification succeeded: the dedicated Chrome profile bootstrap created `%LOCALAPPDATA%\clock-repair-system\linelib-poc\lineoa-storage.json` with safe result `verified=true`, `cookie_count=11`, and `bot_count=1`; storage-only lineoa restoration succeeded, and the existing `LINELibAdapter.from_storage` integration also initialized successfully with `bot_count=1` without calling any send function. No authentication values or LINE identifiers were written to repository output. The previous non-sending auth-storage blocker is resolved. If the session expires, re-login only through the dedicated browser profile and rerun the bootstrap/verify utility; never place auth storage or cookies in the repository, chat, or logs. The next step is a controlled E2E real LINE send as a new Task and requires fresh explicit user approval. No real LINE message, deploy, push, schema change, environment change, or production change occurred in this Task. Reply UI also requires approval. The old DC bridge remains implemented but is not LINE sender architecture; DC removal is not done.
