...
このdrill-down（段階絞り込み）を実現するためのCalとPartCategoryMaster（標準部品カテゴリ）/機能グループ/PartNameMaster（標準部品名マスタ）の関連モデルは、既存schemaと検索実装を確認してから確定する。推測でFKや新規マスタを追加しない。
...
2. Cal連動の部品候補絞り込みに必要な既存データ構造を調査する
3. 機能グループ相当の中間階層が必要なカテゴリと、既存schemaへの影響を確認する
4. 既存の部品検索・検索ワード生成実装を確認する
5. PartNameTerm（部品用語マスタ）のschema案を作る
6. PDF投入後の用語抽出・出典管理フローを設計する
7. 追加部品のkey（キー）、日本語名、英語代表名、検索別名、多言語別名、categoryKey（カテゴリキー）、movementTarget（ムーブメント対象）、movementPosition（上下位置）を確定する
8. Codexへ調査Taskを渡す
9. 必要に応じてschema変更または暫定static定義を選ぶ
10. `src/lib/part-input-options.ts` のみを中心に追加する場合は、PartNameTerm（部品用語マスタ）設計と矛盾しないようにする
11. seedを実行して重複・categoryKey不整合・partType不整合がないか確認する
12. TypeScript検証を行う
13. 画面または取得処理で内装部品候補に追加部品が出るか確認する
