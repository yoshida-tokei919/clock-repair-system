...
実装
→ local自動確認（型・test・必要に応じてPlaywright等）
→ commit
→ migration/env差分確認
...
`02_PRODUCT_ROADMAP.md` で全体のMVP優先順位と現在の開発順序を確認し、現在Taskがどの段階に属するか把握してから開始する。
...
- カタリは、コンテキストが肥大化してからではなく、Taskの安全な区切りで先回りして「ここで新しいチャットへ移るのがおすすめ」とユーザーへ提案する。
- 特に、長い調査や大量のツール出力が続いた後、1 Taskのcommit/deployが完了した後、次の大きなTaskへ入る前、または引き継ぎプロンプトをまだ確実に作れる段階で切替を判断する。
- コンテキスト上限に近づいて引き継ぎ文を作れなくなるまで作業を続けない。
- チャット切替を提案する時は、現在作業を安全なcheckpointで止め、次チャット用の引き継ぎプロンプトを同じターンで作成する。
- 引き継ぎプロンプトには必ず、repo path、必須読込ファイル3点、現在branch / git status、現在Task、完了済み、未完了、Task外の未commit差分、直近commit、次に行う具体的操作、ユーザー承認が必要な工程を含める。
...
- ユーザーの手動実画面確認は原則完了条件にせず、Codex/AI側でTypeScript、lint、test、build、git diff --check、および必要に応じてPlaywright等のブラウザ自動確認を行う。プリンタ・スマホ実機・外部サービス本人操作・実顧客送信などAIだけで安全に確認できない項目のみユーザー確認を依頼する。
