...
- [ ] 取消して未請求に戻すボタンが表示される
- [ ] 確認ダイアログが表示される
- [ ] 取消実行後 `Invoice.status = void` になる
- [ ] 紐づく `Repair.invoiceId` が `null` に戻る
- [ ] `/invoices` で同じ対象月・取引先を選ぶと再抽出される
- [ ] 取消済み請求書が一覧で「取消済み」と表示される
- [ ] 保存済みPDFが削除されていない
- [ ] 請求書番号が再利用されない
- [ ] `paid` の請求書は取消不可
...
- 請求書取消後、対象案件が未請求として再抽出できる。

### 請求書取消 No-Go条件

- 取消しても `Repair.invoiceId` が残る。
- `paid` 請求書を取消できてしまう。
- 取消時にPDFや送信履歴が削除される。
- 請求書番号が巻き戻る。
