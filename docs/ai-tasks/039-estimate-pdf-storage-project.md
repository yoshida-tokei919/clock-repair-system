...
- B2B共有ページURLをLINEで送る
- PDF添付はしない
- 共有ページ内PDFボタンから保存済みPDFを開く

納品書:

- 時計と一緒に紙で渡す帳票
- オンライン共有・LINE送信・PDF添付は不要
- PDFファイル保存も現時点では不要寄り
- 必要なら管理画面から都度PDF表示して印刷する

請求書:

- 月次請求として生成する
- PDFはSupabase Storageに保存する方向
- LINEで請求書共有URLを送る
- PDF添付はしない
...
- `EstimateServerDocument.ts` は保存済み見積PDFの正本として扱う
- 保存済み見積PDFの表ヘッダーは削除しない
- 保存済み見積PDFの列幅はヘッダー行と明細行で共通化する
- 保存済み見積PDFのヘッダー表記は `金額(税抜)` に統一する
