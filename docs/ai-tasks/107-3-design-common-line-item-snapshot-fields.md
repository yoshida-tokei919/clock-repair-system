...
> 作業名・部品名・B2B/B2C表示名・価格表示フラグは、帳票・共有ページ・PublicCase表示時にマスタを後読みせず、Repair明細 / EstimateItem / RepairLineItem 側のスナップショットを正とする。
> PublicCaseは、その明細スナップショットから生成する公開用の別スナップショットである。
