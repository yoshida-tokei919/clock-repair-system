...
- 請求書は原則として完全削除しない。
- 誤発行・作り直しが必要な場合は、請求書を取消済みにする。
- 取消時には、紐づく `Repair.invoiceId` を `null` へ戻し、対象案件を未請求状態に戻す。
- 保存済みPDF、`publicToken`、`sentAt`、送信履歴相当の情報は削除しない。
- 請求書番号は再利用しない。

### 月次請求・都度請求の両立

- 同じ取引先・同じ対象月に複数請求書が存在すること自体は許容する。
- 理由は、月次請求だけでなく、都度請求の取引先もあるため。
- したがって、同月複数請求書をDB制約で禁止しない。
- 誤って分割作成した場合は、不要な請求書を取消して未請求に戻し、必要に応じて再作成する。
...
本当は YD-001 + YD-002 をまとめたい場合:
YI-001を取消
→ YD-001側のRepair.invoiceIdがnullに戻る
→ 再度5月分を抽出
→ YD-001 + YD-002で新しい請求書を作成
```
...
- `Invoice` が存在しなければ404。
- `paid` は取消不可。
- `void` / `canceled` は再取消不可。
- transaction内で `Invoice.status = "void"` にする。
- transaction内で `Repair.updateMany({ where: { invoiceId }, data: { invoiceId: null } })` を実行する。
- 成功時に `releasedRepairCount` を返す。

### UI仕様

- `/documents/invoice/[id]` に「取消して未請求に戻す」ボタンを表示する。
- `paid` / `void` の請求書では表示しない、または取消不可とする。
- 取消実行前に確認ダイアログを出す。
- 取消済み請求書では、詳細画面上部に取消済みメッセージを表示する。
- `/invoices` 一覧では `void` を「取消済み」と表示する。
